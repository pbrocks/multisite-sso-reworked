...
$body_text = __( 'Hang tight ...', 'wp-multisite-sso' );
...
if ( WP_MultiSite_SSO::LOGIN_ACTION === $action ) {
	...
	$body_text = sprintf( __( '%1$s %2$s', 'wp-multisite-sso' ), $sso_options['login_message'], $body_text );
} else {
	// $body_text = sprintf( __( 'Logging out of network sites. %s', 'wp-multisite-sso' ), $body_text );
	$body_text = sprintf( __( '%1$s %2$s', 'wp-multisite-sso' ), $sso_options['logout_message'], $body_text );
}
 ...
 */
$redirect = apply_filters( 'wp_multisite_sso_redirect', $redirect, $action, $current_user );
?>
<html>
	<head>
		<?php
	...
	<body class="<?php echo implode( ' ', $body_classes ); ?>">
		<div id="login">
			...
			<h4 style="color:green;" id="nav"><?php echo esc_html( $body_text ); ?></h4>
		</div>
	</body>
</html>
