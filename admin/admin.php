...
class WP_MultiSite_SSO_Admin {
	...
	public static function admin_init() {
		...
		add_settings_field(
			'login_message', __( 'Login Message', 'wp-multisite-sso' ), array( 'WP_MultiSite_SSO_Admin', 'login_message_callback' ), WP_MultiSite_SSO::SETTINGS_SLUG
		);

		add_settings_field(
			'logout_message', __( 'Logout Message', 'wp-multisite-sso' ), array( 'WP_MultiSite_SSO_Admin', 'logout_message_callback' ), WP_MultiSite_SSO::SETTINGS_SLUG
		);
	...
	public static function sanitize_settings( $settings ) {
		...
		if ( isset( $settings['login_message'] ) ) {
			$new_settings['login_message'] = esc_attr( $settings['login_message'] );
		}

		if ( isset( $settings['logout_message'] ) ) {
			$new_settings['logout_message'] = esc_attr( $settings['logout_message'] );
		}
		return $new_settings;
	}
	 ...
	 * Callback to display a textarea so a user can enter custom CSS for the SSO login page
	 */
	public static function login_message_callback() {
		$sso_options = get_option( WP_MultiSite_SSO::SETTINGS_SLUG );
		$login         = isset( $sso_options['login_message'] ) ? $sso_options['login_message'] : '';

		printf( '<textarea id="login_message" name="wp_multisite_sso_settings[login_message]" cols="50" rows="7">%s</textarea>', esc_attr( $login ) );
	}

	/**
	 * Callback to display a textarea so a user can enter custom CSS for the SSO login page
	 */
	public static function logout_message_callback() {
		$sso_options = get_option( WP_MultiSite_SSO::SETTINGS_SLUG );
		$logout         = isset( $sso_options['logout_message'] ) ? $sso_options['logout_message'] : '';

		printf( '<textarea id="logout_message" name="wp_multisite_sso_settings[logout_message]" cols="50" rows="7">%s</textarea>', esc_attr( $logout ) );
	}
}
